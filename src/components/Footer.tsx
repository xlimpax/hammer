...
export default function Footer() {
  ...
  return (
    <footer id="contact" className="bg-[var(--color-brand-gray)] pt-20 pb-10 border-t border-black/5">
      <div className="container mx-auto px-6 md:px-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
          ...
          <div className="flex flex-col gap-6">
            <Link href="/">
              <span className="text-3xl font-black tracking-tighter text-[var(--color-brand-white)]">
                HAMMER<span className="text-[var(--color-brand-orange)]">.</span>
              </span>
            </Link>
            <p className="text-[var(--color-brand-light)] text-sm leading-relaxed">
              ...
              {mounted && Object.entries(socialLinks || {}).map(([platform, url]) => {
                ...
                return (
                  <a 
                    ...
                    className={`w-10 h-10 rounded-full bg-black/5 flex items-center justify-center text-[var(--color-brand-light)] hover:text-white transition-all duration-300 hover:scale-110 ${socialColors[platform] || "hover:bg-[var(--color-brand-orange)]"}`}
                  >
                    <IconComponent />
                  </a>
                );
          ...
          {/* Contact Details */}
          <div>
            <h4 className="text-[var(--color-brand-white)] font-bold mb-6 text-lg">Contact Us</h4>
            <ul className="flex flex-col gap-4 text-sm text-[var(--color-brand-light)]">
        ...
        <div className="border-t border-black/10 pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-[var(--color-brand-light)] text-sm">
            &copy; {new Date().getFullYear()} HAMMER. All rights reserved.
          </p>
          <div className="flex items-center gap-6 text-sm text-[var(--color-brand-light)]">
            <Link href="/admin/login" className="hover:text-[var(--color-brand-orange)] transition-colors">Admin Login</Link>
            <Link href="#" className="hover:text-[var(--color-brand-white)] transition-colors">Privacy Policy</Link>
            <Link href="#" className="hover:text-[var(--color-brand-white)] transition-colors">Terms of Service</Link>
          </div>
        </div>
      </div>
    </footer>
